...
int ceil_Search(int arr[], int s, int e, int n)
{
	if(n == arr[0])
	{
		return 0;
	...
	{
		if(mid - 1 >= s && n > arr[mid - 1])
		{
			return mid;
		}
		else
		{
			return ceil_Search(arr, s, mid - 1, n);
		}
	}
	else
	{
		if(mid + 1 <= e && n <= arr[mid + 1])
		{
			return mid + 1;
		}
		else
		{
			return ceil_Search(arr, mid + 1, e, n);
		}
	}
}
...
//Floor_and_Ceiling_in_sorted_array.cpp