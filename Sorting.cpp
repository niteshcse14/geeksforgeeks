...
void solve()
{
	...
	vect.pb(mp(4, 6));
	vect.pb(mp(3, 7));
	sort(vect.b(), vect.e(), sortbysecdescending);
	vector<pair<int ,int>>::IT i;
	for(i = vect.b(); i != vect.e(); i++)
	{
		vector<pair<int ,int>>::IT j = i;
		while(*j != *i)
		{
			j++;
			i++;
	...
	vector<pair<int ,int>>::IT j;
	for(j = vect.b(); j != vect.e(); j++)
	{
		cout<<(j->f)<<"   "<<(j->s)<<"\n";
	}
}
