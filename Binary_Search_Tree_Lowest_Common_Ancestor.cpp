...
void BST_LCA(Tree *node, int aa, int bb)
{
	if(node->data < aa && node->data < bb)
	{
		BST_LCA(node->right,aa ,bb);
	}
	else if(node->data > aa && node->data > bb)
	{
		BST_LCA(node->left,aa ,bb);
	}
	else
	{
		p_i(node->data);
...
//Binary_Search_Tree_Lowest_Common_Ancestor.cpp